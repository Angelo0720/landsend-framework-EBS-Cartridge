...
def createJob = freeStyleJob ("${ENVIRONMENT} - Node2 Fetch Tracefile")

createJob.with {
  parameters {
    choiceParam('USER_DUMP_DEST', ['/u01/oracle/app/r12prd/11204/diag/rdbms/r12prd/AESGDEV2/trace'], '')
...
If you don't specify the filename, it will fetch the directory listing instead.''')
    choiceParam('DB_LOGIN', ['bxdp_orar12@bxdpdb02.bos1.aesgdevapp'], '')
    ...
    choiceParam('PROFILE_FILE_NAME', ['AESGDEV2.env'], '')
  }
