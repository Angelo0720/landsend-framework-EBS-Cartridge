...
def createJob = freeStyleJob ("${ENVIRONMENT} - Install R12 FND Files")
...
createJob.with {
  parameters {
    ...
    stringParam('EXECUTION_SCRIPTNAME', './xxcu_upload_dynamic_fnd.sh.ALL', '')
  }
  configure { project ->
...
./scripts/pushFolders --target $APP_SERVER \\
      ...
      --exclude ".svn" \\
      --exec-remote ". ~/.bash_profile; . ~/APP_PROFILE; chmod -R +x  $XXCU_TOP/bin; cd $XXCU_TOP/bin; echo $APPS_PASSWORD | STOP_ON_ERROR=${STOP_ON_ERROR} SKIPCOUNT=${SKIPCOUNT} LOG_LEVEL=${LOG_LEVEL} ${EXECUTION_SCRIPTNAME} UPLOAD"
    ''')
  }
}